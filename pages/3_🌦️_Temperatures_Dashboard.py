...
max_temp_city = temps_df.loc[temps_df['AvgTemperatureCelsius'].idxmin(), 'City']
max_temp_date = temps_df.loc[temps_df['AvgTemperatureCelsius'].idxmin(), 'Date']
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"] == city]            # TODO
        city_df_period = city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]     # TODO
        plt.hist(city_df_period["AvgTemperatureCelsius"], bins=20, alpha=0.7, label=city)                # TODO
